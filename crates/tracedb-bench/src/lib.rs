...
fn record_write_timing_samples(timing: &WritePathTiming, samples: &mut BTreeMap<String, Vec<f64>>) {
    ...
    for (name, value) in [
        ...
        (
            ...
            timing.store_apply_validate_identity_ms,
        ),
        (
            "store_apply_validate_vector",
            timing.store_apply_validate_vector_ms,
        ),
        ("store_apply_key", timing.store_apply_key_ms),
        ("store_apply_fields", timing.store_apply_fields_ms),
        (
            "store_apply_finalize_identity",
            timing.store_apply_finalize_identity_ms,
        ),
        ("store_apply_features", timing.store_apply_features_ms),
        ("store_apply_install", timing.store_apply_install_ms),
